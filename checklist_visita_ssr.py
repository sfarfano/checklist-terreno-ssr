...
# Sección de administrador con exportación
with st.expander("🔐 Acceso administrador para exportación"):
    ...
    if clave == "admin1234":
        ...
        if not df.empty:
            ...
            with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                resumen[["% Completado"]].sort_values("% Completado", ascending=False).to_excel(writer, sheet_name='Resumen', index=True)

            st.download_button(
                label="📥 Descargar Resumen General en Excel",
                data=output.getvalue(),
                file_name="resumen_checklist_ssr.xlsx",
...
if menu == "Registro de Checklist":
    ...
    if nombre_ssr != "Selecciona un SSR...":
        ...
        for item in checklist_items:
            # Usamos un key único por SSR e ítem
            respuesta = st.checkbox(item, key=f"{nombre_ssr}_{item}")
            respuestas[item] = respuesta

        if st.button("Guardar Registro", key=f"guardar_{nombre_ssr}"):
        ...
        if not registros_ssr.empty:
            ...
            if st.checkbox("✏️ Editar este registro"):
                ...
                for item in checklist_items:
                    ...
                    ediciones[item] = st.checkbox(item, value=bool(valor_actual))
